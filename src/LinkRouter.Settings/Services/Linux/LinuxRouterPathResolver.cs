...
internal sealed class LinuxRouterPathResolver : IRouterPathResolver
{
    public bool TryGetRouterExecutable([NotNullWhen(true)] out string? path)
    {
        const string executableName = "LinkRouter.Launcher";
        ...
        // Try standard Linux installation locations
        var searchPaths = new[]
        {
            // User-local install (preferred)
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".local", "bin", executableName),
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".local", "share", "LinkRouter", "bin", executableName),

            // XDG data home
            Path.Combine(
                Environment.GetEnvironmentVariable("XDG_DATA_HOME")
                    ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".local", "share"),
                "LinkRouter", "bin", executableName),

            // System-wide install locations
            Path.Combine("/usr", "local", "bin", executableName),
            Path.Combine("/opt", "LinkRouter", executableName),
        };
